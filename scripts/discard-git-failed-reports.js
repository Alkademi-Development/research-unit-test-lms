...
async function askRemoveFailedScreenshoots() {
    ...
    rl.question(clc.bold(clc.yellow("⚠  Apakah anda yakin ingin discard semua perubahan file report yang gagal ? (Y/t) ")), inputConfirm => {
        ...
        if(inputConfirm.trim().toLowerCase() === 't') {
            ...
            git.status((err, status) => {
                ...
                status.files.forEach(file => {
                    if(file.path.includes("testResults/reports") && file.path.includes("fail")) files.push(file.path);
                });
                ...
                if(files.length > 0) {
                    ...
                    async function discardChanges(files) {
                        try {
                          ...
                          console.log(clc.bold(clc.green("Oke, terimakasih telah mengkonfirmasi semua perubahan file report hasil test yang gagal telah berhasil di discard 👌\n")));
                        } catch (error) {
