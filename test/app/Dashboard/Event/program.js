...
Waktu Event Load Selesai (loadEventEnd): (${performanceTiming.loadEventEnd - navigationStart}) milliseconds ( ${((performanceTiming.loadEventEnd - navigationStart) / 1000)} seconds )
    ...
    BROWSERS.forEach(browser => {
        ...
        users.forEach(userData => {
            ...
            switch (user.kind) {
                case 0:
                    it.skip(`SUPER ADMIN - Create a new Event Program from browser ${browser}`, async () => {
                        ...
                        try {
                            ...
                            await driver.executeScript(`return Array.from(document.querySelectorAll("ul.navbar-nav li")).find(value => {
                                const innerSpan = value.querySelector("i.ri-rocket-line ~ span");
                                return innerSpan && innerSpan.innerText.toLowerCase().includes("event");
                            }).querySelector("#Event a").click()`)

                            // Aksi sleep
                            await driver.sleep(3000)

                            // Aksi menekan tombol '+ Event'
                            ...
                            let amountOfPrimary = faker.number.int({ min: 0, max: 3 });
                            let amountOfRequirement = faker.number.int({ min: 0, max: 3 });
                            let amountOfTerm = faker.number.int({ min: 0, max: 3 });
                            let primacy = [];
                            let requirements = [];
                            let terms = []; 
                            let link = 'https://google.com';
                            /** Aksi mengisi input name */
                            await driver.findElement(By.id("Judul Program *")).sendKeys(title)
                            ...
                            await driver.sleep(3000);
                            /** Aksi mengisi input keunggulan program */
                            for (let index = 0; index < amountOfPrimary; index++) {
                                primacy.push({
                                    title: 'Positive',
                                    desc: faker.lorem.paragraph()
                                })
                                /** Aksi mengisi input title primacy */
                                let inputsTitle = await driver.executeScript(`return document.querySelectorAll(".card .card-body label[for='Keunggulan Program'] ~ div input[placeholder='Judul']");`);
                                await inputsTitle[index].sendKeys(primacy[index].title)
                                await driver.sleep(2000)
                                /** Aksi mengisi input description primacy */
                                let inputsDesc = await driver.executeScript(`return document.querySelectorAll(".card .card-body label[for='Keunggulan Program'] ~ div textarea")`)
                                await inputsDesc[index].sendKeys(primacy[index].desc)
                                await driver.sleep(2000)
                                if(amountOfPrimary > 0) await driver.executeScript(`return Array.from(document.querySelectorAll("button.btn-primary")).find(value => value.innerText.toLowerCase().includes("keunggulan")).click()`)
                            }
                            /** Aksi mengisi input requirements */
                            for (let index = 0; index < amountOfRequirement; index++) {
                                requirements.push({
                                    title: 'Positive',
                                    desc: faker.lorem.paragraph()
                                })
                                /** Aksi mengisi input title requirements */
                                let inputsTitle = await driver.executeScript(`return document.querySelectorAll(".card .card-body label[for='Persyaratan Program'] ~ div input[placeholder='Judul']");`);
                                await inputsTitle[index].sendKeys(requirements[index].title)
                                await driver.sleep(2000)
                                /** Aksi mengisi input description requirements */
                                let inputsDesc = await driver.executeScript(`return document.querySelectorAll(".card .card-body label[for='Persyaratan Program'] ~ div textarea")`)
                                await inputsDesc[index].sendKeys(requirements[index].desc)
                                await driver.sleep(2000)
                                if(amountOfRequirement > 0) await driver.executeScript(`return Array.from(document.querySelectorAll("button.btn-primary")).find(value => value.innerText.toLowerCase().includes("persyaratan")).click()`)
                            }
                            /** Aksi mengisi input requirements */
                            for (let index = 0; index < amountOfTerm; index++) {
                                terms.push({
                                    title: 'Positive',
                                    desc: faker.lorem.paragraph()
                                })
                                /** Aksi mengisi input title terms */
                                let inputsTitle = await driver.executeScript(`return document.querySelectorAll(".card .card-body label[for='Ketentuan Program'] ~ div input[placeholder='Judul']");`);
                                await inputsTitle[index].sendKeys(terms[index].title)
                                await driver.sleep(2000)
                                /** Aksi mengisi input description terms */
                                let inputsDesc = await driver.executeScript(`return document.querySelectorAll(".card .card-body label[for='Ketentuan Program'] ~ div textarea")`)
                                await inputsDesc[index].sendKeys(terms[index].desc)
                                await driver.sleep(2000)
                                if(amountOfTerm > 0) await driver.executeScript(`return Array.from(document.querySelectorAll("button.btn-primary")).find(value => value.innerText.toLowerCase().includes("ketentuan")).click()`)
                            }
                            /** Aksi mengisi input link program */
                            await driver.findElement(By.id("Link Program*")).sendKeys(link)
                            await driver.sleep(2000)
                            ...
                            let isAllFilled = await Promise.all([
                                await driver.findElement(By.id("Judul Program *")).getAttribute('value'),
                            ...
                            customMessages = [
                                await alertWarning == null && isAllFilled ? 'Successfully created a new event program ✅' : 'Failed to create a new event program ❌'
                            ]
                    ...
                    it(`SUPER ADMIN - Edit the Event Program from browser ${browser}`, async () => {
                        ...
                        try {
                            ...
                            let description = faker.lorem.paragraph();
                            /** Aksi mengisi input name */
                            await driver.findElement(By.id("Judul Program *")).clear();
                            await driver.findElement(By.id("Judul Program *")).sendKeys(title)
                            await driver.sleep(2000);
                            /** Aksi mengisi input description */
                            let textEditorTinyMce = await driver.wait(until.elementLocated(By.css('.tox-tinymce')));
                            ...
                            // Expect results and add custom message for addtional description
                            customMessages = [
                                await alertWarning == null && isAllFilled ? 'Successfully updated the event program ✅' : 'Failed to update the event program ❌'
                            ]
                            expect(await alertWarning).to.be.null
                            expect(isAllFilled).to.be.true
                    ...
                    it.skip(`SUPER ADMIN - Check the pagination of Table List Event Program from browser ${browser}`, async () => {
                    ...
                    it.skip(`SUPER ADMIN - Check the next page of pagination of Table List Event Program from browser ${browser}`, async () => {
                    ...
                    it.skip(`SUPER ADMIN - Check the last page number of pagination of Table List Event Program from browser ${browser}`, async () => {
                    ...
                    it.skip(`SUPER ADMIN - Choose a page number of pagination of Table List Event Program from browser ${browser}`, async () => {
                    ...
                    it.skip(`SUPER ADMIN - Check the previous page of pagination of Table List Event Program from browser ${browser}`, async () => {
                    ...
                    it.skip(`SUPER ADMIN - Check the first page of pagination of Table List Event Program from browser ${browser}`, async () => {
                    ...
                    it.skip(`SUPER ADMIN - Delete the Event Program from browser ${browser}`, async () => {
                        ...
                        try {
                            ...
                            customMessages = [
                                await tableBody != await newTableBody ? '- Successfully deleted the event program ✅' : '- Failed to delete the event program ❌'
                            ];
                ...
                case 1:
                    it.skip(`ADMIN - Create a new Event Program from browser ${browser}`, async () => {
                    ...
                    it(`ADMIN - Edit the Event Program from browser ${browser}`, async () => {
                    ...
                    it.skip(`ADMIN - Check the pagination of Table List Event Program from browser ${browser}`, async () => {
                    ...
                    it.skip(`ADMIN - Check the next page of pagination of Table List Event Program from browser ${browser}`, async () => {
                    ...
                    it.skip(`ADMIN - Check the last page number of pagination of Table List Event Program from browser ${browser}`, async () => {
                    ...
                    it.skip(`ADMIN - Choose a page number of pagination of Table List Event Program from browser ${browser}`, async () => {
                    ...
                    it.skip(`ADMIN - Check the previous page of pagination of Table List Event Program from browser ${browser}`, async () => {
                    ...
                    it.skip(`ADMIN - Check the first page of pagination of Table List Event Program from browser ${browser}`, async () => {
                    ...
                    it.skip(`ADMIN - Delete the Event Program from browser ${browser}`, async () => {
                ...
                case 6:
                    it.skip(`CONTENT WRITER - Create a new Event Program from browser ${browser}`, async () => {
                    ...
                    it(`CONTENT WRITER - Edit the Event Program from browser ${browser}`, async () => {
                    ...
                    it.skip(`CONTENT WRITER - Check the pagination of Table List Event Program from browser ${browser}`, async () => {
                    ...
                    it.skip(`CONTENT WRITER - Check the next page of pagination of Table List Event Program from browser ${browser}`, async () => {
                    ...
                    it.skip(`CONTENT WRITER - Check the last page number of pagination of Table List Event Program from browser ${browser}`, async () => {
                    ...
                    it.skip(`CONTENT WRITER - Choose a page number of pagination of Table List Event Program from browser ${browser}`, async () => {
                    ...
                    it.skip(`CONTENT WRITER - Check the previous page of pagination of Table List Event Program from browser ${browser}`, async () => {
                    ...
                    it.skip(`CONTENT WRITER - Check the first page of pagination of Table List Event Program from browser ${browser}`, async () => {
                    ...
                    it.skip(`CONTENT WRITER - Delete the Event Program from browser ${browser}`, async () => {
